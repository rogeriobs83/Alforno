...
const validateOrder = (body) => {
  ...
  if (
    !["pay_on_fulfillment", "demo_card", "demo_wallet"].includes(
      body.paymentMethod,
    )
  ) {
  ...
  const items = body.items.map((item) => {
    ...
    const size = item.size
      ? validateString(item.size, "Item size", 3, 10)
      : undefined;
...
// ------------------------------------------------------------
// 🧩 EXPRESS CONFIG (JSON, PROXY, SESSÕES)
// ------------------------------------------------------------
...
app.get("/api/addresses", async (request, response) => {
  ...
  if (query.length < 3 || query.length > 100) {
    response
      .status(400)
      .json({
        error: "Enter a postcode or address with at least 3 characters.",
      });
    return;
  }
  ...
  try {
    ...
    if (!addressResponse.ok) {
      console.error(
        `Nominatim lookup failed with HTTP ${addressResponse.status}.`,
      );
      response
        .status(addressResponse.status)
        .json({ error: "Unable to find addresses for that postcode." });
      return;
    }
...
app.post("/api/admin/login", (request, response) => {
  if (!adminPassword || !sessionSecret) {
    response
      .status(503)
      .json({ error: "Administrator access has not been configured." });
    return;
  }
  ...
  request.session.regenerate((error) => {
    if (error) {
      ...
      response
        .status(500)
        .json({ error: "Unable to sign in. Please try again later." });
      return;
    }
...
app.post("/api/admin/logout", requireAdmin, (request, response) => {
  request.session.destroy((error) => {
    if (error) {
      ...
      response
        .status(500)
        .json({ error: "Unable to sign out. Please try again later." });
      return;
    }
...
app.get("/api/admin/reservations", requireAdmin, async (_request, response) => {
  try {
    ...
    response
      .status(500)
      .json({ error: "Unable to load reservations. Please try again later." });
  }
});
  ...
  async (request, response) => {
    ...
    try {
      ...
      response
        .status(500)
        .json({
          error: "Unable to update reservation. Please try again later.",
        });
    }
  },
);
...
app.delete(
  ...
  async (request, response) => {
    ...
    try {
      const reservation = await app.locals.reservations.findOne({
        _id: new ObjectId(id),
      });
      ...
      if (!isPastReservation(reservation)) {
        response
          .status(400)
          .json({ error: "Only past reservations can be deleted." });
        return;
      }
      ...
      response
        .status(500)
        .json({
          error: "Unable to delete reservation. Please try again later.",
        });
    }
  },
);
...
app.post("/api/orders", async (request, response) => {
  try {
    ...
    response
      .status(201)
      .json({ id: result.insertedId.toString(), status: "pending" });
  } catch (error) {
    ...
    response
      .status(500)
      .json({ error: "Unable to submit your order. Please try again later." });
  }
});

// ------------------------------------------------------------
// 📅 RESERVAS — CRIAR RESERVA
// ------------------------------------------------------------
app.post("/api/reservations", async (request, response) => {
  try {
    ...
    response
      .status(500)
      .json({
        error: "Unable to save your reservation. Please try again later.",
      });
  }
});

// ------------------------------------------------------------
// 🚀 START DO SERVIDOR + CONEXÃO MONGO
// ------------------------------------------------------------
const start = async () => {
...
// ------------------------------------------------------------
// 🔁 KEEP-ALIVE (PING NO RENDER)
// ------------------------------------------------------------
setInterval(() => {
  fetch("https://seu-backend.onrender.com/").catch((error) => {
